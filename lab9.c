...
{
    int index;
	index = hash(record->id, tableSize);// call the hash function to get the index
    if(hashTable[index].p == NULL){// if the RecordType at that index is NULL
		hashTable[index].p = record;// set 'record' equal to the HashType pointer in the table at index
        // we use . since it is array format 
        // (hasTable + index)->p (format if we wanna use ->)
	}
    //handle collition
	else{
...
int main(void)
{
    ...
    struct HashType *hash_table = calloc(hashTableSize, sizeof(struct HashType));
	
	int x;
	for(x=0; x<recordSz; x++){
		insertRecord(hash_table, &pRecords[x], hashTableSize);
        //(pRecords + 1) same thing as &pRecords[x];
	}
